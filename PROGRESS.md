...
- **Backups (2026-09-18):** restore mechanism tested and confirmed working — `npx supabase db dump --db-url "$DIRECT_URL" -f backup.sql`, restored into a throwaway local Postgres container, all tables came back with correct columns/types/defaults/FKs/indexes and RLS enabled (15 at the time of this test — the 14 from the foundation phase plus `course_invitations`, added by the Core CRUD branch in between; not a discrepancy, just later timing). This validates a manual backup/restore procedure independent of Supabase's own opaque automated daily backups (free tier, no PITR, still untested on Supabase's side). Worth a recurring automated version of this dump eventually (`resilience-builder`'s domain) — not urgent with an empty DB.
...
None currently open — the prior set (role-change lag, CAPTCHA, dashboard settings, `npx getdesign` vetting, admin scope, backups) is fully resolved; see "Decisions & assumptions" below and the "Resolved 2026-09-18" note in the Foundation phase log above.
