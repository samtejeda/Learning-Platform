...
## In progress
- **Core CRUD — courses & lectures** on branch `worktree-academy-backend` (backend-builder). Commits 1–3 of 6 done (see the commit log above). Remaining: 4 lectures (create/upload/finalize/publish/reorder/delete), 5 `GET /api/lectures/[id]/stream` + `POST /api/lectures/[id]/progress` + anti-scrub policy + student lecture page/minimal player, 6 docs reconcile + audit self-check. Plan file: `~/.claude/plans/let-s-begin-with-plan-wobbly-token.md`.
